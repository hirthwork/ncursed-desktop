...
VERSION="4.0.0-ALPHA"
SRC_URI="mirror://apache/lucene/java/${VERSION}/${PN}-${VERSION}-src.tgz"
LICENSE="Apache-2.0"
SLOT="4.0"
KEYWORDS="~amd64 ~x86"
IUSE=""
...
java_prepare() {
	...
	java-pkg_jar-from --build-only --into lib ant-ivy:2
...
src_compile() {
	...
	ANT_TASKS="none" ANT_OPTS="-Duser.home=${T}" eant -lib lib -Dversion=${PV} \
...
src_test() {
	...
	EANT_GENTOO_CLASSPATH="junit ant-core ant-junit" ANT_TASKS="ant-junit" ANT_OPTS="-Duser.home=${T}" eant -lib lib test-core
}
...
src_install() {
	...
	if use contrib; then
		for i in `find  build/contrib -name \*-${VERSION}.jar`
		do
		    j=${i##*/}
			java-pkg_newjar $i ${j%%-${VERSION}.jar}.jar
		done
	fi
	if use doc; then
		dohtml -r docs/* || die
